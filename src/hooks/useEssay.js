...
export const useEssay = () => {
    ...
    useEffect(() => {
        if (currentUser) {
            setEssayState(prev => ({ ...prev, userId: currentUser.uid }));
            fetchSavedEssays(currentUser.uid);
        }
    }, [currentUser]);
    ...
    const startNewEssay = () => {
        setEssayState({ ...INITIAL_STATE, userId: currentUser?.uid });
        setMessages([
            {
                role: 'ai',
                type: 'essay_welcome',
        ...
        if (currentUser) fetchSavedEssays(currentUser.uid);
    };
