...
export const usePowerPoint = () => {
    ...
    useEffect(() => {
        if (currentUser) {
            setPowerPointState(prev => ({ ...prev, userId: currentUser.uid }));
            fetchSavedPowerPoints(currentUser.uid);
        }
    }, [currentUser]);
    ...
    const startNewPowerPoint = () => {
        setPowerPointState({ ...INITIAL_STATE, userId: currentUser?.uid });
        setFiles({ main: null, sources: [], template: null, copycat: null });
        setMessages([
            {
                role: "assistant",
        ...
        if (currentUser) fetchSavedPowerPoints(currentUser.uid);
    };
