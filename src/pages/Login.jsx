...
import { GraduationCap, ArrowRight, Eye, EyeOff, Mail, Lock, Loader2 } from 'lucide-react';
        ...
        <div className="min-h-screen bg-[var(--bg-color)] text-[var(--text-primary)] font-sans relative flex items-center justify-center p-6 overflow-hidden transition-colors duration-300">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
                <motion.div
                    ...
                    className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-indigo-600/15 rounded-full blur-[120px] mix-blend-screen"
                />
                <motion.div
                    animate={{ rotate: -360, scale: [1, 1.2, 1] }}
                    transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                    className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[60%] bg-purple-600/15 rounded-full blur-[120px] mix-blend-screen"
                />
            </div>
            ...
            >
                ...
                <div className="glass-panel p-10 rounded-[2rem] border border-[var(--glass-border)] relative backdrop-blur-3xl bg-white/60 dark:bg-black/40 shadow-2xl">
                    ...
                    <motion.div variants={itemVariants} className="text-center mb-8">
                        <h2 className="text-3xl font-extrabold mb-2 font-outfit tracking-tight">Welcome back</h2>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <motion.div variants={itemVariants} className="pt-2">
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full btn-primary py-4 text-lg font-semibold shadow-indigo-500/25 shadow-xl flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed"
                            >
                    ...
                    <motion.div variants={itemVariants} className="mt-8 text-center text-sm text-[var(--text-secondary)]">
                        ...
                        <Link to="/signup" className="text-indigo-500 font-semibold hover:text-indigo-400 transition-colors">
                            Create an account
                        </Link>
                    </motion.div>
                </div>
